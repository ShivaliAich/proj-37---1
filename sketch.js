...
function setup() {
  ...
  gameover.x = monkey.x-100;
  gameover.y = monkey.y-700;
  //monkey.debug = true;
...
function draw() {
 ...
 // monkey.collide(edge(4))
  ...
  if(gamestate === PLAY){
    ...
    textSize(20);
    fill("black");
    text("Press the Space key to make the monkey jump",monkey.x,monkey.y-300);
  ...
  console.log("love");
    ...
    if(gamestate === END){
      textSize(20);
      fill("black");
      text("Press the up arrow to restart the game",monkey.x,monkey.y-300);
      monkey.scale = 0.2;
